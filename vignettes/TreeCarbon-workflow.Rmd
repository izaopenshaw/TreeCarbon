...
results <- fc_agc(
  ...
  dbh = example_data$dbh,
  height = example_data$height,
...
results_na <- fc_agc(
  name = trees_with_na$name,
  dbh = trees_with_na$dbh,
  height = trees_with_na$height,
  type = trees_with_na$type
)
...
if (requireNamespace("BIOMASS", quietly = TRUE)) {
  ...
  result_custom <- fc_agc(
    name = "Oak",
    dbh = 45,
    height = 18,
    type = "broadleaf",
    nsg = wd$meanWD  # Custom wood density
  )
...
system.time({
  results_std <- fc_agc(
    name = trees$name[1:10],
    dbh = trees$dbh[1:10],
    height = trees$height[1:10],
    type = trees$type[1:10],
    rich_output = FALSE
  )
})
