# App1 - WCC Tree Carbon Calculator
              ...
              fluidRow(
                ...
                box(title = "Data Input", width = 4, status = "primary", solidHeader = TRUE,
                    ...
                    selectInput("method", "Carbon Conversion Method:",
                                ...
                                selected = "Thomas"),
                    hr(),
                    checkboxInput("append_results", "Append to existing results", value = TRUE),
...
server <- function(input, output, session) {
  ...
  observeEvent(input$calculate, {
    ...
    withProgress(message = "Calculating...", value = 0, {
      ...
      if (!is.null(newResult)) {
        if (isTRUE(input$append_results)) {
          appData$results <- rbind(appData$results, newResult)
        } else {
          appData$results <- newResult
        }
  ...
  create_plot <- reactive({
    ...
    if (input$plot_type == "carbon_bar") {
      ...
      # Stacked biomass (base R reshape - no tidyr)
      req(all(c("crownbiomass_t", "stembiomass_t", "rootbiomass_t") %in% colnames(plot_data)))

      plot_data_long <- reshape(plot_data,
        varying = c("crownbiomass_t", "stembiomass_t", "rootbiomass_t"),
        v.names = "biomass",
        timevar = "component",
        times = c("crownbiomass_t", "stembiomass_t", "rootbiomass_t"),
        direction = "long"
      )
      ...
      if (input$show_errors) {
        ...
        if (all(c("sig_stembiomass", "sig_crownbiomass", "sig_rootbiomass") %in% colnames(plot_data))) {
          ...
          p <- p + geom_errorbar(data = plot_data,
                                 aes(x = factor(seq_len(nrow(plot_data))),
