...
# ==============================================================================
# Replicates calculations from WCC Carbon Assessment Protocol v2.0 using
# TreeCarbon functions.
#
# SIGNPOSTING: Method reference (functions, averaging): WCC_method_reference.R
#
# References:
...
for (i in 1:nrow(comp1_trees)) {
  ...
  tree_carbon <- fc_agc_error(name = comp1_species,
                              dbh = comp1_trees$dbh_cm[i],
                              height = comp1_trees$height_m[i],
                              type = comp1_type, method = "Matthews2",
...
for (i in 1:nrow(comp2_trees)) {
  tree_carbon <- fc_agc_error(name = comp2_trees$species[i],
                              dbh = comp2_trees$dbh_cm[i],
                              height = comp2_trees$height_m[i],
                              type = comp2_type, method = "Matthews2",
                              re_dbh = 0.025, re_h = 0.05)
...
for (i in 1:nrow(app6_trees)) {
  tree_carbon <- fc_agc_error(name = app6_trees$species[i],
                              dbh = app6_trees$dbh_cm[i],
                              height = app6_trees$height_m[i],
                              type = app6_type, method = "Matthews2",
                              re_dbh = 0.025, re_h = 0.05)
